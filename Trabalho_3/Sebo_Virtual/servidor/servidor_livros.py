...
def processar_requisicao(cliente_socket: socket.socket) -> None:
    ...
    try:
        # Usar um unico stream por conexao para evitar inconsistencias de buffer
        ...
        while True:
            ...
            if len(tamanho_bytes) < 4:
                print("[servidor] Tamanho de comando inválido", file=sys.stderr)
                break

            tamanho_comando = struct.unpack("!I", tamanho_bytes)[0]
            comando_bytes = arquivo.read(tamanho_comando)
            if len(comando_bytes) < tamanho_comando:
                print("[servidor] Comando incompleto recebido", file=sys.stderr)
                break
